...
# Force pester v4
Install-Module -Repository 'PSGallery' -Scope 'CurrentUser' -Name 'Pester' -RequiredVersion 5.7.1 -SkipPublisherCheck
...
if ($env:CI_WINDOWS -eq $true) {
    ...
    if (!(Test-Path "$env:SCOOP_HELPERS\lessmsi\lessmsi.exe")) {
        Start-FileDownload 'https://github.com/activescott/lessmsi/releases/download/v2.1.1/lessmsi-v2.1.1.zip' -FileName "$env:SCOOP_HELPERS\lessmsi.zip"
